...
<script language='Javascript'>
	...
	function cancelar(f){
					window.location="navegadorMaquinas.php";
	}
</script>
...
02 de Julio de 2008
-->
<form name="form1" method="post" action="eliminarMaquinas.php">
<?php 
...
<input type="hidden" name="datos" value="<?php echo $datos; ?>">
	<h3 align="center" style="background:white;font-size: 14px;color: #d20000;font-weight:bold;">Confirmacion de Eliminacion de Maquinas </h3>
    ...
    <?php
		...
		for($i=0;$i<$n;$i++){	
				...
				$sw=0;			
				$sql=" select  *  from hojas_rutas_detalle_maquinaria  where cod_maquina='".$cod_maquina."'";			
				...
				if($sw==0){
					/****************************/
						if($datosEliminar<>""){
							$datosEliminar=$datosEliminar.",".$cod_maquina;
						}else{
							$datosEliminar=$cod_maquina;
						}
					/****************************/
				}else{
					/***************************/
						if($datosNoEliminar<>""){
								$datosNoEliminar=$datosNoEliminar.",".$cod_maquina;
						}else{
							$datosNoEliminar=$cod_maquina;
						}
					/**************************/		
				}
...
<h3 align="center" style="background:white;font-size: 11px;color:#d20000;">Registros que no pueden ser Elimandos</h3>
	<table width="80%" align="center" cellpadding="1" cellspacing="1" bgColor="#cccccc">
		...
		<?php
			...
			for($j=0;$j<$num;$j++){	
				$cod_maquina=$vectordatosNoEliminar[$j];
...
<h3 align="center" style="background:white;font-size: 11px;color:#d20000;">Registros que pueden ser Elimandos</h3>
	<table width="80%" align="center" cellpadding="1" cellspacing="1" bgColor="#cccccc">
		...
		<?php
			...
			for($j=0;$j<$num;$j++){	
				$cod_maquina=$vectordatosEliminar[$j];
				$sql=" select desc_maquina, cod_estado_registro, cod_usuario_registro,";
				$sql.=" fecha_registro, cod_usuario_modifica, fecha_modifica ";
				$sql.=" from maquinaria ";
				$sql.=" where cod_maquina=".$cod_maquina;
