...
function paginar(f)
{	
var param="?descBancoB="+f.descBancoB.value;
	param+="&pagina="+f.pagina.value;
		location.href='navegadorBancos.php'+param;		
}
...
{	
	...
	else
	{
		...
		{//alert("editarBanco.php?cod_banco="+cod_registro);
			window.location="editarBanco.php?cod_banco="+cod_registro;
		}
	}
}
...
function eliminar(f)
{
	...
	else
	{
			...
			//window.location ="listaEliminarBancos.php?datos="+datos;			
	}
}
</script></head>
<body bgcolor="#FFFFFF">
...
02 de Julio de 2008
-->
<h3 align="center" style="background:#FFFFFF;font-size: 14px;color: #E78611;font-weight:bold;">BANCOS 
...
<form name="form1" method="post" action="registrarBanco.php">
<?php 
	require("conexion.inc");
	include("funciones.php");
	$descBancoB=$_GET['descBancoB'];
...
<?php	
	...
	$sql_aux=" select count(*) from bancos";
	if($descBancoB<>""){
		$sql_aux.=" where desc_banco like '%".$descBancoB."%'";
	}
	...
	while($dat_aux=mysql_fetch_array($resp_aux)){
		...
		$sql=" select cod_banco, desc_banco,  cod_estado_registro, ";
...
?> 
		<tr bgcolor="#FFFFFF">	
			<td><input type="checkbox" name="cod_banco" value="<?php echo $cod_banco;?>"></td>	
    		<td><?php echo $desc_banco;?></td>
...
<td colspan="3"><input type="text" name="descBancoB" id="descBancoB" size="30" class="textoform" value="<?php echo $descBancoB;?>" ></td>
