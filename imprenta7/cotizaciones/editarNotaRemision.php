...
<title>EDICION NOTA REMISION</title>
...
function atras(f){
			window.location="navegadorNotasRemision.php";
}
...
<body  bgcolor="#FFFFFF">
<form method="post"  name="form1" action="guardaEditarNotaRemision.php">
...
<?php 	
	...
	/***********************DATOS DE NOTA DE REMISION****************************/
	$sql=" select  cod_gestion, nro_nota_remision,";
	$sql.=" fecha_nota_remision, obs_nota_remision, cod_hoja_ruta,";
	$sql.=" cod_estado_nota_remision, recibido_por, cod_usuario_entregado_por ";
	$sql.=" from  notas_remision";		
	//$sql=" select fecha_nota_remision, cod_usuario_nota_remision, ";
	//$sql.=" obs_nota_remision, cod_hoja_ruta, cod_estado_nota_remision, recibido_por ";
	//$sql.=" from notas_remision ";
	$sql.=" where cod_nota_remision='".$cod_nota_remision."'";
	$resp= mysqli_query($enlaceCon,$sql);
	$dat=mysqli_fetch_array($resp);
		$cod_gestion_nota_remision=$dat[0];
		$nro_nota_remision=$dat[1];
		$fecha_nota_remision=$dat[2];
		$obs_nota_remision=$dat[3];
		$cod_hoja_ruta=$dat[4];
		$cod_estado_nota_remision=$dat[5];
		$recibido_por=$dat[6];
		$codusuarioentregadopor=$dat[7];
...
?>
<input type="hidden" name="cod_nota_remision" value="<?php echo $cod_nota_remision; ?>">
...
<?php	
	...
	/************************************/	
		...
		/********************DATOS COTIZACION********************/
			...
			$dat2=mysqli_fetch_array($resp2);
				...
				$cod_gestion_cotizacion=$dat2[1];
				$cod_cliente=$dat2[2];		
				$fecha_cotizacion=$dat2[3];	
				/***********GESTION********/
					$sql3="select gestion  from gestiones where cod_gestion='".$cod_gestion_cotizacion."'";
	...
	?>
	<h3 align="center" style="background:white;font-size: 14px;color: #E78611;font-weight:bold;">EDICION DE NOTA DE REMISION </h3>
	<h3 align="center" style="background:white;font-size: 14px;color: #E78611;font-weight:bold;">No. <?php echo $nro_nota_remision."/".$gestionNotaRemision;?><?php echo "<br> Fecha:".$fecha_nota_remision;?></h3>
    ...
    <table  cellSpacing="1" cellPadding="2" width="90%" bgColor="#cccccc" border="0" id="tabla" align="center">
		...
		<tr bgcolor="#FFFFFF">
     		<td>Observaciones</td>
     		<td colspan="5"><?php echo $obs_hoja_ruta;?></td>
		</tr-->	
		...
		<tr bgcolor="#FFFFFF">
     		...
     		<td colspan="5">
				...
				<?php
				 ...
				 ?>
				 		<?php if($cod_usuario==$codusuarioentregadopor){?>
		...
		<tr bgcolor="#FFFFFF">
     		<td>Recibido por </td>
     		<td colspan="5"><input type="text" name="recibido_por"  value="<?php echo $recibido_por;?>"class="textoform" size="40"></td>
		</tr>
		<tr bgcolor="#FFFFFF">
     		<td>Observaciones</td>
     		<td colspan="5"><textarea name="obs_nota_remision" cols="60" rows="2"><?php echo $obs_nota_remision;?></textarea></td>
		</tr>		
		<tr class="titulo_tabla">
	...
	<?php
		...
		while($dat=mysqli_fetch_array($resp)){
			...
			$sql6=" select count(*)";
			$sql6.=" from notas_remision_detalle";
			$sql6.=" where cod_nota_remision='".$cod_nota_remision."'";
			$sql6.=" and cod_cotizacion='".$cod_cotizacion."'";
			$sql6.=" and cod_cotizaciondetalle='".$cod_cotizaciondetalle."'";
			$resp6=mysqli_query($enlaceCon,$sql6);
			$swPerteneceNotaRemision=0;
			while($dat6=mysqli_fetch_array($resp6)){
				$swPerteneceNotaRemision=$dat6[0];
			}
		...
		<tr bgcolor="#FFFFFF">		     		
		   <td align="right">
		   <input type="checkbox" name="cod_cotizaciondetalle"  value="<?php echo $cod_cotizaciondetalle;?>" <?php if($swPerteneceNotaRemision==1){ echo "checked='checked'";}?>>
		   </td>
		   <td>
		...
		<td colspan="2" valign="top">
			...
			<?php
				...
				$sql8=" select cantidad ";
				$sql8.=" from notas_remision_detalle ";
				$sql8.=" where cod_cotizacion='".$cod_cotizacion."' and cod_cotizaciondetalle='".$cod_cotizaciondetalle."'";
				$sql8.=" and cod_nota_remision='".$cod_nota_remision."'";
			...
			?>
				...
				<td  align="center"><input type="text" name="cantidad" value="<?php echo $cantidad; ?>" size="8" class="textoform"></td>
			</tr>
			</table>
		</td>
