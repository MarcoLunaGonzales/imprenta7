	...
	while ($dat=mysqli_fetch_array($resp)){
		...
		/**********************Fin Datos de Cliente******************************/	
			...
			$this->SetY(39);
			 ...
			 $this->ln(1);
 			 $this->Cell(0,3,"No. ".$nro_cotizacion."/".$gestion,0,1,'C',false);
			 $this->ln(1);
			 $this->Cell(0,4,"FECHA: ".$fechaCotizacionVectoAux[2]."/".$fechaCotizacionVectoAux[1]."/".$fechaCotizacionVectoAux[0],0,1,'C',false);	
...
*/
			...
			$this->Text($valorX+85,82,"DESCRIPCIÓN");
			...
			$this->Line(18,84,18,225); 
			$this->Line(48,84,48,225);
			$this->Line(168,84,168,225);
			$this->Line(200,84,200,225);
	...
	while ($dat=mysqli_fetch_array($resp)){
		...
		if($sw==1 && $cod_sumar==1 ){
			...
			$this->SetY($valorY+226);
			$this->SetX($valorX+20);
			$this->Line(18,226,18,231);
			$this->Line(200,225,200,231);
			$this->Line(18,231,200,231);
			$this->Cell(141,6,"Son:".$suma_literal."00/100 Bolivianos.",0,1,'L',false);
			$swTotal=1;
		}
		...
		if($swTotal==1){
			...
			$this->Text(20,235,"Validez de la oferta ".$dias_validez." días. - Forma de Pago:".$forma_pago);
			if($tiempo_entrega==0){
			$this->Text(20,239,"Tiempo de Entrega: A convenir");
			}else{
				$this->Text(20,239,"Tiempo de Entrega: ".$tiempo_entrega." días hábiles.");
			}
		}else{
			$this->SetFont('Arial','IB',8);
			$this->Text(20,230,"Validez de la oferta ".$dias_validez." días. - Forma de Pago:".$forma_pago);
			if($tiempo_entrega==0){
				$this->Text(20,235,"Tiempo de Entrega: A convenir");
			}else{
				$this->Text(20,235,"Tiempo de Entrega: ".$tiempo_entrega." días hábiles.");
			}
		}
