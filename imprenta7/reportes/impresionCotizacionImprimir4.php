...
/**********************DATOS DE CABECERA*********************/
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		/**********************Fin Datos de Cliente******************************/	
			...
			$this->Text($valorX+163,$valorY+27,$fechaCotizacionVectoAux[2]);
			$this->Text($valorX+176,$valorY+27,$fechaCotizacionVectoAux[1]);
			$this->Text($valorX+187,$valorY+27,$fechaCotizacionVectoAux[0]);	
			$this->SetFont('Arial','B',9);
			$this->SetXY($valorX+160,$valorY+40);
    	    $this->Cell(0,4,'Página '.$this->PageNo().' de '.' {nb}',0,1,'R',false);
			$this->SetFont('Arial','B',16);
			$this->Text($valorX+85,$valorY+45,"No. ".$nro_cotizacion."/".$gestion);
			
			$this->SetFont('Arial','B',10);
			$this->SetTextColor(0,0,0);
			$this->Text($valorX+55,$valorY+62,$nombre_cliente);
			$this->Text($valorX+55,$valorY+69,$direccion_cliente);
			$this->Text($valorX+55,$valorY+76,$telefono_cliente);
			$this->Text($valorX+105,$valorY+76,"Celular:".$celular_cliente);		
			$this->Text($valorX+150,$valorY+76,"Fax:".$fax_cliente);
			$this->SetY(102);
