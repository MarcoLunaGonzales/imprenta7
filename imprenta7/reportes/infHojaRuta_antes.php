...
/**********************DATOS DE CABECERA*********************/
	...
	$facturas="";
	$sqlAux=" select fhr.cod_factura, f.nro_factura, f.fecha_factura, f.monto_factura ";
	$sqlAux.=" from factura_hojaruta fhr, facturas f ";
	$sqlAux.=" where fhr.cod_factura=f.cod_factura  and f.cod_est_fac<>2 ";
	$sqlAux.=" and fhr.cod_hoja_ruta=".$cod_hoja_ruta;
	...
	if($facturas<>""){
		...
		/**********************Fin Datos de Cliente******************************/	
			...
			$this->SetTextColor(0,0,0);
			$this->Text(85,13,"DETALLE DE GASTOS");
			$this->SetFont('Arial','B',10);
			$this->SetXY(12,22);
			...
			$this->SetFont('Arial','',10);
			$this->SetXY(23,42);
			$this->Cell(0,5,$nro_cotizacion."/".$gestion,0,0,'L');			
			$this->SetFont('Arial','B',10);
			$this->SetXY(42,42);
			$this->Cell(0,5,'FECHA COT:',0,0,'L');
			$this->SetFont('Arial','',10);
			$this->SetXY(64,42);
...
/*********************************CUERPO DE COTIZACION****************************************/
		...
		while ($dat=mysqli_fetch_array($resp)){
			...
			if($nro_nr_item>0){
				...
				$notas_remision_item=$notas_remision_item." NR.:".$nro_nota_remision."/".$gestion_nota_remision." (".$fecha_nota_remision.") Cant.: ".$cantidad.";";	
				}			
...
$sw=1;
	...
	$pdf->Cell(20,4,number_format(($monto_hojaruta-$descuento_cotizacion),2),1,1,'R',false);		
	...
	$sql2=" select pd.cod_moneda, pd.monto_pago_detalle, p.fecha_pago ";
			 	...
			 	$sql2.=" and pd.cod_hoja_ruta=".$cod_hoja_ruta;
				$resp2 = mysqli_query($enlaceCon,$sql2);
				$acuenta_hojaruta=0;
				while($dat2=mysqli_fetch_array($resp2)){
	...
	$pdf->Cell(20,4,number_format((($monto_hojaruta-$descuento_cotizacion)-($acuenta_hojaruta)),2),0,1,'R',false);	
	...
	$sqlAux.=" and pd.cod_hoja_ruta=".$cod_hoja_ruta;
	...
	$sqlAux.=" and s.cod_hoja_ruta=".$cod_hoja_ruta;
	...
	$pdf->SetFont('Arial','B',7);	
	$pdf->SetX(12);
	$pdf->Cell(180,4,"Descripcion de Gasto",1,0,'C',false);	
	$pdf->SetX(192);
	$pdf->Cell(15,4,"Monto Bs",1,1,'C',false);
	...
	////////////////GASTOS EXTRAS/////////////////////////
	$gastosExtras=0;
	$sqlAux=" select ghr.cod_gasto, ghr.monto_gasto, g.desc_gasto ";
	$sqlAux.=" from gastos_hojasrutas ghr, gastos g ";
	$sqlAux.=" where ghr.cod_gasto=g.cod_gasto ";
	$sqlAux.=" and ghr.cod_hoja_ruta=".$cod_hoja_ruta;
	$respAux = mysqli_query($enlaceCon,$sqlAux);
	while($datAux=mysqli_fetch_array($respAux)){	
		...
		$pdf->SetFont('Arial','',6);	
		$pdf->SetX(12);
		$pdf->Cell(180,4,$desc_gasto,1,0,'R',false);	
		$pdf->SetX(192);
		$pdf->SetFont('Arial','',6);
	...
	$pdf->SetX(192);
	$pdf->SetFont('Arial','',6);
	$pdf->Cell(15,4,number_format(($costoTotalSalidas+$gastosExtras),2)." Bs",1,1,'R',false);
	
 
 	$pdf->ln();
 	$pdf->SetFont('Arial','B',6);	
	$pdf->SetX(12);
	$pdf->Cell(180,4,"INGRESOS PARA INVENTA",0,0,'R',false);	
	$pdf->SetX(192);
	$pdf->SetFont('Arial','',6);
	$pdf->Cell(15,4,number_format(($monto_hojaruta-$descuento_cotizacion)-($costoTotalSalidas+$gastosExtras),2)." Bs",1,1,'R',false);
