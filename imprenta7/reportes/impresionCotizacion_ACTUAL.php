...
//Cabecera de página
	function Header()
	{	
	$this->Image('ULTIMOMembretado.jpg',0,0,214);
...
/**********************DATOS DE CABECERA*********************/
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		/**********************Fin Datos de Cliente******************************/	
			...
			$this->Text(87,22,"COTIZACIÓN");
	
			$this->SetFont('Arial','B',9);
			$this->SetXY($valorX+165,$valorY+59);
    	    $this->Cell(0,4,'Página '.$this->PageNo().' de '.' {nb}',0,1,'R',false);
			$this->SetFont('Arial','B',16);
			$this->Text($valorX+90,$valorY+30,"No. ".$nro_cotizacion."/".$gestion);
			$this->Text($valorX+83,$valorY+38,"FECHA: ".$fechaCotizacionVectoAux[2]."/".$fechaCotizacionVectoAux[1]."/".$fechaCotizacionVectoAux[0]);

			
			$this->SetFont('Arial','B',10);
			$this->SetTextColor(0,0,0);
			$this->Text($valorX+20,$valorY+46,"CLIENTE:");
			$this->Text($valorX+45,$valorY+46,$nombre_cliente);
			$val_Y_cabecera=46;
			...
			$this->SetTextColor(0,0,0);
			$val_Y_cabecera=$val_Y_cabecera+5;		
			$this->Text($valorX+20,$val_Y_cabecera,"DIRECCIÓN:");
			if($cod_unidad<>"" and $cod_unidad<>0 and $cod_unidad<>NULL){
			...
			$this->Line(18,65,200,65);
			$this->Text($valorX+20,71,"CANTIDAD");
			$this->Text($valorX+85,71,"DESCRIPCIÓN");
			$this->Text($valorX+175,71,"IMPORTE");		
			$this->Line(18,73,200,73);
			
			$this->Line(18,65,18,73);
			$this->Line(48,65,48,73);
			$this->Line(168,65,168,73);
			$this->Line(200,65,200,73);
			
			
			$this->Line(18,73,18,225);
			$this->Line(48,73,48,225);
			$this->Line(168,73,168,225);
			$this->Line(200,73,200,225);
			$this->Line(18,225,200,225);			
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		if($sw==1 && $cod_sumar==1 ){
			...
			$this->SetY($valorY+225);
			$this->SetX($valorX+20);
			$this->Line(18,219,18,231);
			$this->Line(200,219,200,231);
			$this->Line(18,231,200,231);
		...
		$this->Text(20,234,"Validez de la oferta ".$dias_validez." días. - Forma de Pago:".$forma_pago);
		if($tiempo_entrega==0){
			$this->Text(20,237,"Tiempo de Entrega: A convenir");
		}else{
			$this->Text(20,237,"Tiempo de Entrega: ".$tiempo_entrega." días hábiles.");
		}
		
		$this->Line(18,239,115,239);
		$this->Line(18,264,115,264);
		$this->Line(18,239,18,264);
		$this->Line(115,239,115,264);
		$this->SetFont('Arial','IB',6);
		$this->Text(20,242,"Impresiones Offset de:");
		$this->Text(20,245,"- Memorias, Afiches,");	
		$this->Text(20,248,"- Agendas, Trípticos,");
		$this->Text(20,251,"- Brochures, Volantes,");
		$this->Text(20,254,"- Semanarios, Libros,");
		$this->Text(20,257,"- Semanarios, Libros,");
		$this->Text(20,260,"- Revistas,");
		$this->Text(20,263,"- y otros.");
		
		$this->Text(65,242,"Diseño Gráfico Publicitario.");		
		
		$this->Text(65,245,"Gigantografias.");
		$this->Text(65,248,"CDs Interactivos.");	
		$this->Text(65,251,"Copia de CDs y DVDs.");
		$this->Text(65,254,"Serigrafía de:");
		$this->Text(65,257,"- Boligrafos,");
		$this->Text(65,260,"- Credenciales,");
		$this->Text(65,263,"- Llaveros, etc.");
			
		$this->Line(125,239,200,239);
		$this->Line(125,264,200,264);
		$this->Line(125,239,125,264);
		$this->Line(200,239,200,264);
	...
	/***************************DATOS DE CABECERA********************************/
		...
		$this->SetFont('Arial','B',9);
		$this->Text(140,252,"..........................................................");
		$this->Text(153,256,"Elaborado por");
		$this->SetY($valorY+258);
