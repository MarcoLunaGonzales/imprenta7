...
//Cabecera de página
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		while($dat2=mysql_fetch_array($resp2)){
			$gestionHojaRuta=$dat2['gestion'];
			$gestionNombreHojaRuta=$dat2['gestion_nombre'];
		}		
		/************FIN GESTION********************/		
		$nro_hoja_ruta=$dat[1];
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		$resp5=mysql_query($sql5);
		while ($dat5=mysql_fetch_array($resp5)){
			$gestionCotizacion=$dat5['gestion'];
			$gestionNombreCotizacion=$dat5['gestion_nombre'];
		}			
	...
	/***************************FIN FACTURAS*************************************/	
			...
			$this->Cell(0,6,"CONFIRMACIÓN DE PEDIDO ",0,1,'C',false);
			$this->SetFont('Arial','B',13);
			$this->SetXY($valorX+2,$valorY+20);
			$this->Cell(0,6,"No. ".$nro_hoja_ruta." / ".$gestionNombreHojaRuta,0,1,'C',false);
			...
			$this->SetTextColor(0,0,0);		
			$this->SetXY($valorX+160,$valorY+21);
    	    $this->Cell(160,4,'Página '.$this->PageNo().' de '.' {nb}',0,1,'L',false);

			$this->SetFont('Arial','B',9);
			$this->SetTextColor(0,0,0);				
			$this->Text($valorX+16,$valorY+38,"PROVEEDOR:");	
			$this->SetFont('Arial','',9);			
			$this->Text($valorX+55,$valorY+38,"JENISSE PUBLICIDAD ARTISTICA");	
			...
			$val_Y_cabecera=$val_Y_cabecera+5;		
			$this->Text($valorX+16,$val_Y_cabecera,"ENCARGADO:");
			$this->SetFont('Arial','',9);
			$this->Text($valorX+55,$val_Y_cabecera,$usuarioHojaRuta);
			...
			$val_Y_cabecera=$val_Y_cabecera+5;	
			$this->SetFont('Arial','B',9);
			$this->Text($valorX+16,$val_Y_cabecera,"FECHA DE PEDIDO:");	
			$this->SetFont('Arial','',9);
			$this->Text($valorX+55,$val_Y_cabecera,$fechaHojaRutaVectoAux[2]."/".$fechaHojaRutaVectoAux[1]."/".$fechaHojaRutaVectoAux[0]			);
			...
			//$this->Text($valorX+175,$val_Y_cabecera,$nro_cotizacion."/".$gestionNombreCotizacion);
			...
			$val_Y_cabecera=$val_Y_cabecera+5;
			$this->SetFont('Arial','',9);
			$this->Text($valorX+16,$val_Y_cabecera,"Sírvanse por este medio suministrarnos de los siguientes productos y/o servicios de acuerdo a los artes ya aprobados.");
	...
	while ($dat=mysql_fetch_array($resp)){
		...
		if($sw==1 && $cod_sumar==1 ){
			...
			$this->Cell(20,6,"Total : ".$numero_formato,0,1,'R',false);
...
$sw=1;
	...
	$sql=" select  cod_cliente  from cotizaciones ";
	$sql.=" where  cod_cotizacion='".$cod_cotizacion."'";
	$resp=mysql_query($sql);
	while ($dat=mysql_fetch_array($resp)){
		...
		$pdf->SetFont('Arial','B',9);
	$pdf->Cell(0,6,$nombre_cliente,0,1,'R',false);
	...
	$pdf->Cell(0,6,"FIRMA ",0,1,'C',false);
	$pdf->ln();
	$pdf->Cell(0,6,"------------------------------------ ",0,1,'C',false);
	$pdf->Cell(0,6,"NOMBRE COMPLETO ",0,1,'C',false);
	$pdf->ln();
	$pdf->Cell(0,6,"------------------------------------ ",0,1,'C',false);
	$pdf->Cell(0,6,"SELLO ",0,1,'C',false);
